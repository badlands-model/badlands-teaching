...
def view1Step(tin, flow, sea, scaleZ=40,  maxZ=100, maxED=20,flowlines=False):
    ...
    """
    lv = lavavu.Viewer(border=False, axis=False, background="white")
    ...
    lv.control.Range(command='scale z', range=(1,scaleZ), step=1., value=20)
    lv.control.Range(command='background', range=(0,1), step=0.1, value=1.1)
...
def viewTime(folder,steps=100,it=2,scaleZ=40, maxZ=100,maxED=20,flowlines=False):
    ...
    lv.control.Range(command='scale z', range=(1,scaleZ), step=1., value=20)
    lv.control.Range(command='background', range=(0,1), step=0.1, value=1.0)
